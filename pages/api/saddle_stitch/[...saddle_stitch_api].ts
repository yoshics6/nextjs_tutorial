  ...
  async (req: NextApiRequest, res: NextApiResponse, next: any) => {
    ...
    try {
      ...
      const [response]: any = await connection.query(
        ...
        sadd_text_paper LIKE ? OR sadd_printing LIKE ? OR sadd_cover_coating LIKE ? OR sadd_text_coating LIKE ? OR sadd_1000 LIKE ? OR sadd_2000 LIKE ? OR sadd_3000 LIKE ? 
...
router.put(
  ...
  async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      , sadd_text_coating , sadd_1000 , sadd_2000 , sadd_3000 , sadd_4000 , sadd_5000} = fields;
      ...
      " sadd_cover_coating,sadd_text_coating,sadd_1000,sadd_2000,sadd_3000,sadd_4000,sadd_5000) "+
      " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [
        ...
        sadd_1000,sadd_2000,sadd_3000,sadd_4000,sadd_5000
      ]);
...
router.post(
  ...
  async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    form.parse(req, async (err, fields, files) => {
      const { sadd_type , sadd_finished_size , sadd_cover , sadd_text , sadd_cover_paper , sadd_text_paper , sadd_printing , sadd_cover_coating
        , sadd_text_coating , sadd_1000 , sadd_2000 , sadd_3000 , sadd_4000 , sadd_5000 , sadd_id} = fields;
      await connection.query("UPDATE saddle_stitch SET sadd_type = ? , sadd_finished_size = ? , sadd_cover = ? , sadd_text = ? , sadd_cover_paper = ? , "+
      " sadd_text_paper = ? , sadd_printing = ? , sadd_cover_coating = ? , sadd_text_coating = ? , sadd_1000 = ? , sadd_2000 = ? , sadd_3000 = ? ,"+
      " sadd_4000 = ? , sadd_5000 = ? WHERE sadd_id = ?", [
        sadd_type , sadd_finished_size , sadd_cover , sadd_text , sadd_cover_paper , sadd_text_paper , sadd_printing , sadd_cover_coating , sadd_text_coating , 
        sadd_1000 , sadd_2000 , sadd_3000 , sadd_4000 , sadd_5000 , sadd_id
      ]);
      res.status(200).json({ status: "success" });
    });
  }
...
async function importExcelUser(url: string) {
  ...
  await Promise.all(
    data.map(async (rows: any, index: any) => {
      ...
      const sadd_printing = rows["Printing"];
      const sadd_cover_coating = rows["Cover Coating"];
      const sadd_text_coating = rows["Text Coating"];
      const sadd_1000 = `${rows["1000"]}`;
      const sadd_2000 = `${rows["2000"]}`;
      const sadd_3000 = `${rows["3000"]}`;
      const sadd_4000 = `${rows["4000"]}`;
      const sadd_5000 = `${rows["5000"]}`;
