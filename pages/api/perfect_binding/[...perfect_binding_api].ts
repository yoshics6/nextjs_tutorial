  ...
  async (req: NextApiRequest, res: NextApiResponse, next: any) => {
    ...
    try {
      ...
      const [response]: any = await connection.query(
        ...
        perf_text_paper LIKE ? OR perf_printing LIKE ? OR perf_cover_coating LIKE ? OR perf_text_coating LIKE ? OR perf_1000 LIKE ? OR perf_2000 LIKE ? OR perf_3000 LIKE ? 
...
router.put(
  ...
  async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      , perf_text_coating , perf_1000 , perf_2000 , perf_3000 , perf_4000 , perf_5000} = fields;
      ...
      " perf_cover_coating,perf_text_coating,perf_1000,perf_2000,perf_3000,perf_4000,perf_5000) "+
      " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [
        ...
        perf_1000,perf_2000,perf_3000,perf_4000,perf_5000
      ]);
...
router.post(
  ...
  async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    form.parse(req, async (err, fields, files) => {
      const { perf_type , perf_finished_size , perf_cover , perf_text , perf_cover_paper , perf_text_paper , perf_printing , perf_cover_coating
        , perf_text_coating , perf_1000 , perf_2000 , perf_3000 , perf_4000 , perf_5000 , perf_id} = fields;
      await connection.query("UPDATE perfect_binding SET perf_type = ? , perf_finished_size = ? , perf_cover = ? , perf_text = ? , perf_cover_paper = ? , "+
      " perf_text_paper = ? , perf_printing = ? , perf_cover_coating = ? , perf_text_coating = ? , perf_1000 = ? , perf_2000 = ? , perf_3000 = ? ,"+
      " perf_4000 = ? , perf_5000 = ? WHERE perf_id = ?", [
        perf_type , perf_finished_size , perf_cover , perf_text , perf_cover_paper , perf_text_paper , perf_printing , perf_cover_coating , perf_text_coating , 
        perf_1000 , perf_2000 , perf_3000 , perf_4000 , perf_5000 , perf_id
      ]);
      res.status(200).json({ status: "success" });
    });
  }
...
async function importExcelUser(url: string) {
  ...
  await Promise.all(
    data.map(async (rows: any, index: any) => {
      ...
      const perf_printing = rows["Printing"];
      const perf_cover_coating = rows["Cover Coating"];
      const perf_text_coating = rows["Text Coating"];
      const perf_1000 = `${rows["1000"]}`;
      const perf_2000 = `${rows["2000"]}`;
      const perf_3000 = `${rows["3000"]}`;
      const perf_4000 = `${rows["4000"]}`;
      const perf_5000 = `${rows["5000"]}`;
